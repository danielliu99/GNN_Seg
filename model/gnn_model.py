...
class GNN:
    def __init__(self,model_type,hyperparameters,train_dataset, 
        ...
        self.optimizer=torch.optim.AdamW([{'params': self.net.parameters(), 'initial_lr': 5e-4}],lr=hyperparameters.lr,weight_decay=hyperparameters.w_decay)

        self.lr_decay = torch.optim.lr_scheduler.ExponentialLR(self.optimizer, hyperparameters.lr_decay, last_epoch=last_epoch, verbose=False)
        ...
        self.train_loader = DataLoader(train_dataset,batch_size=BATCH_SIZE,shuffle=True,
                                       ...
                                       pin_memory=True) if train_dataset is not None else None
    ...
    def run_epoch(self):
        self.net.train()
        losses=[]
        self.logger.info(f"Last epoch: {self.lr_decay.last_epoch}")
        for batch_mris,batch_graphs,batch_features,batch_labels in self.train_loader:
            ...
            # if self.logger: 
            #     self.logger.info(f"Batch loss: {loss.detach().cpu().numpy()}")
            self.optimizer.zero_grad()
            loss.backward()
            self.optimizer.step()
        self.lr_decay.step()
        epoch_loss = np.mean(losses)
        self.logger.info(f"Epoch loss: {epoch_loss}")
        # return epoch_loss
        return epoch_loss, self.optimizer, self.lr_decay
    ...
    def evaluate(self,dataset:ImageGraphDataset):
        ...
        if self.logger: 
            self.logger.info(f"Start train time evaluation==================>")
