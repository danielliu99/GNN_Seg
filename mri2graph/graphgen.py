...
def extract_supervoxel_statistics(sv_partitioning,voxel_intensities,voxel_labels,num_supervoxels):
    ...
    if(len(voxel_intensities.shape)>3):
        ...
        # sv_feats.append(sv_centroids)
        sv_feats.append(np.log(1 + np.abs(sv_centroids - np.array((240/2, 240/2, 155/2)))))
    ...
    ## get the 'mode' label in a supervoxel as the label of the supervoxel 
    ...
    ## why np.ones: just to get the spatial center, but not considering intensity
    # sv_centroids = np.array(ndimage.center_of_mass(np.ones(sv_partitioning.shape),sv_partitioning,range(0,num_supervoxels)))
...
def build_adjacency_matrix(positions,intensities,k,weighted=True,enforce_regularity=True):
    ## euc_distances: a matrix [len(positions), len(positions)]
...
Similarly, once the nodes have been classified, the predictions can be reverted to an image by assigning each node's label
...
def img2graph(voxel_intensities,voxel_labels,approx_num_nodes=5000,boxiness=0.5,k=10):
    ...
    slic_partitioning = slic(voxel_intensities.astype(np.float64),
                             n_segments=approx_num_nodes,
                             sigma=1,
                             compactness=boxiness,
                             multichannel=multi_channel,
                            #  channel_axis=3 if using scikit-image >= 0.19,
                             convert2lab=False, 
                             start_label=0).astype(np.int16)
    ...
    print("Number of supervoxels of updated partition: ", np.amax(updated_partitioning)+1)
    ...
    nx_graph = nx.from_numpy_array(graph_adjacency)  # from_numpy_matrix
